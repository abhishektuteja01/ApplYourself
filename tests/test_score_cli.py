...
def _make_clean(tmp_path, job_ids=("aaaaaaaa", "bbbbbbbb")):
    ...
    df = pd.DataFrame([{**base, "job_id": j, "vertical": "sap"} for j in job_ids],
...
class TestCoverage:
    ...
    def test_unreadable_batch_is_reported_not_raised(self, tmp_path):
        self._stage(tmp_path, ["a", "b"], {"batch_sap_001.json": ["a"]})
        (tmp_path / "batch_sap_002.json").write_text('[{"job_id": "b"')
        r = coverage(tmp_path)
        assert r["unreadable"] == ["batch_sap_002.json"]
        assert r["missing"] == ["b"]  # still diagnosable
        assert r["staged"] == 1

    def test_non_array_batch_is_reported_not_raised(self, tmp_path):
        self._stage(tmp_path, ["a"], {})
        (tmp_path / "batch_sap_001.json").write_text('{"job_id": "a"}')
        r = coverage(tmp_path)
        assert r["unreadable"] == ["batch_sap_001.json"]
        assert r["staged"] == 0
...
class TestMergeRefusesToClearStaging:
    ...
    def test_merge_keeps_staging_and_exits_nonzero(self, tmp_path, monkeypatch, capsys):
        staging = self._staging(tmp_path, monkeypatch, {
            "batch_sap_001.json": json.dumps([_score("aaaaaaaa")]),
            "batch_sap_002.json": '[{"job_id": "bbbbbbbb"',
        })
        rc = score_cli._cmd_merge(argparse.Namespace(model="t"))
        out = capsys.readouterr().out
        assert rc == 1
        assert "merged=1" in out
        assert "batch_sap_002.json" in out
        assert (staging / "batch_sap_002.json").exists()
        assert (staging / "batch_sap_001.json").exists()
    ...
    def test_merge_is_rerunnable_after_repair(self, tmp_path, monkeypatch, capsys):
        ...
        (staging / "batch_sap_002.json").write_text(json.dumps([_score("bbbbbbbb")]))
        capsys.readouterr()
        rc = score_cli._cmd_merge(argparse.Namespace(model="t"))
        assert rc == 0
...
class TestJudgeRanges:
    ...
    def test_ranges_tile_one_to_n_exactly(self, n, chunk):
        ranges = judge_ranges({"sap": n}, chunk=chunk)
        assert self._rows(ranges, "sap") == list(range(1, n + 1))

    @pytest.mark.parametrize("chunk", [10, 30, 100])
    def test_zero_count_emits_no_range(self, chunk):
        assert judge_ranges({"sap": 0}, chunk=chunk) == []

    def test_all_verticals_zero_emits_nothing(self):
        assert judge_ranges({"sap": 0, "risk_ai": 0, "ai_eng": 0}) == []
    ...
    def test_ranges_are_ascending_and_non_overlapping(self, chunk):
        ranges = judge_ranges({"sap": 250}, chunk=chunk)
        prev_end = 0
        for _, a, b in ranges:
    ...
    def test_only_the_last_chunk_is_short(self, chunk, n):
        sizes = [b - a + 1 for _, a, b in judge_ranges({"sap": n}, chunk=chunk)]
        assert all(s == chunk for s in sizes[:-1])
        assert 0 < sizes[-1] <= chunk

    def test_exact_multiple_leaves_no_short_chunk(self):
        assert judge_ranges({"sap": 200}, chunk=100) == [
            ("sap", 1, 100), ("sap", 101, 200),
        ]

    def test_count_below_chunk_is_one_range(self):
        assert judge_ranges({"sap": 42}, chunk=100) == [("sap", 1, 42)]

    def test_count_of_one_is_a_degenerate_range(self):
        assert judge_ranges({"sap": 1}, chunk=100) == [("sap", 1, 1)]

    def test_default_chunk_is_100(self):
        assert judge_ranges({"sap": 150}) == [("sap", 1, 100), ("sap", 101, 150)]
    ...
    def test_verticals_are_independent_and_never_interleave(self):
        ...
        ranges = judge_ranges({"sap": 150, "risk_ai": 0, "ai_eng": 100})
        assert ranges == [
            ("sap", 1, 100), ("sap", 101, 150), ("ai_eng", 1, 100),
        ]
        assert self._rows(ranges, "sap") == list(range(1, 151))
        assert self._rows(ranges, "ai_eng") == list(range(1, 101))
    ...
    def test_emission_order_follows_counts_insertion_order(self):
        ...
        names = [v for v, _, _ in judge_ranges({"ai_eng": 1, "sap": 1})]
        assert names == ["ai_eng", "sap"]
    ...
    def test_no_collision_in_the_batch_filename_formula_at_default_chunk(self):
        ...
        ranges = judge_ranges({"sap": 1000, "ai_eng": 450}, chunk=100)
...
class TestClearStaging:
    ...
    def test_removes_every_staging_glob(self, tmp_path):
        staging = tmp_path / "staging"
        staging.mkdir()
        for name in ("batch_sap_001.json", "batch_ai_eng_002.json",
                     "unscored.jsonl", "unscored_sap.jsonl",
    ...
    def test_leaves_unrelated_files_alone(self, tmp_path):
        staging = tmp_path / "staging"
        staging.mkdir()
        (staging / "batch_sap_001.json").write_text("x")
        keep = {
            "notes.md": "hand notes",
            "batch_sap_001.json.bak": "a manual repair copy",
    ...
    def test_does_not_recurse_into_subdirs(self, tmp_path):
        ...
        (staging / "archive" / "batch_sap_001.json").write_text("x")
        score_cli.clear_staging(staging)
        assert (staging / "archive" / "batch_sap_001.json").exists()
...
class TestRangesCommand:
    ...
    def test_prints_counts_then_one_line_per_judge(self, tmp_path, monkeypatch, capsys, cfg):
        self._staging(tmp_path, monkeypatch, {"sap": 150, "ai_eng": 0, "risk_ai": 100})
        rc, lines = self._run(capsys)
        assert rc == 0
        assert lines[0] == "sap=150 ai_eng=0 risk_ai=100"
        assert lines[1:] == [
            "range sap 1-100", "range sap 101-150", "range risk_ai 1-100",
        ]
    ...
    def test_line_format_matches_prepare(self, tmp_path, monkeypatch, capsys, cfg):
        ...
        self._staging(tmp_path, monkeypatch, {"sap": 5})
        _, lines = self._run(capsys)
        assert lines[1] == "range sap 1-5"
    ...
    def test_missing_per_vertical_file_counts_zero(self, tmp_path, monkeypatch, capsys, cfg):
        ...
        self._staging(tmp_path, monkeypatch, {"sap": 20})
        rc, lines = self._run(capsys)
        assert rc == 0
        assert lines[0].startswith("sap=20 ")
        assert lines[1:] == ["range sap 1-20"]
    ...
    def test_empty_file_emits_no_range(self, tmp_path, monkeypatch, capsys, cfg):
        staging = self._staging(tmp_path, monkeypatch, {})
        (staging / "unscored_sap.jsonl").write_text("")
        rc, lines = self._run(capsys)
        assert rc == 0
    ...
    def test_blank_lines_are_not_counted(self, tmp_path, monkeypatch, capsys, cfg):
        ...
        staging = self._staging(tmp_path, monkeypatch, {"sap": 3})
        (staging / "unscored_sap.jsonl").write_text(
            (staging / "unscored_sap.jsonl").read_text() + "\n\n")
        _, lines = self._run(capsys)
        assert lines[0].startswith("sap=3 ")
        assert lines[1] == "range sap 1-3"
    ...
    def test_counts_match_the_files_split_wrote(self, tmp_path, monkeypatch, capsys, cfg):
        ...
        _write_jsonl(staging / "unscored.jsonl",
                     [{"job_id": f"{i:08x}", "vertical": "sap"} for i in range(7)]
                     + [{"job_id": f"b{i:07x}", "vertical": "risk_ai"} for i in range(2)])
        split_counts = split_by_vertical(staging / "unscored.jsonl")
        _, lines = self._run(capsys)
        assert lines[0] == " ".join(f"{v}={n}" for v, n in split_counts.items())
        assert lines[1:] == ["range sap 1-7", "range risk_ai 1-2"]
    ...
    def test_exposed_as_a_cli_subcommand(self, tmp_path, monkeypatch, capsys, cfg):
        ...
        self._staging(tmp_path, monkeypatch, {"sap": 12})
        assert score_cli.main(["ranges"]) == 0
        out = capsys.readouterr().out
        assert "range sap 1-12" in out
